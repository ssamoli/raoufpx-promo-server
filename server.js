...
app.post('/track-share', function(req, res) {
  ...
  pushActivity('Invite Copied', code + ' via ' + method);
  return res.json({ ok: true });
});
...
app.get('/invite/status', function(req, res) {
  ...
  var invites = Array.from(codeStore.values())
    .filter(function(c) { return c.parentCode === parentCode && c.source === 'referral'; })
    .map(function(c) {
      return {
        code:        c.code,
        ...
        friendName:  c.friendName || null,
        hasBooking:  !!c.bookingSubmittedAt,
        inviteLevel: c.inviteLevel || 1,
      };
    });
  ...
  return res.json({
    ...
    usedInvites:     invites.filter(function(c){ return c.status === 'redeemed'; }).length,
    sharedInvites:   invites.filter(function(c){ return c.sharedAt; }).length,
    rewardPoints:    rewardPoints,
    rewardsUnlocked: rewardsUnlocked,
    canInvite:       (parentEntry ? (parentEntry.inviteLevel || 0) : 0) < 2,
  });
});
...
app.get('/admin/invite-tree/:code', requireAdmin, function(req, res) {
  ...
  function buildNode(code) {
    ...
    var children = all
      ...
      .map(function(c) {
        ...
        for (var j = 0; j < leadStore.length; j++) {
          if (leadStore[j].code === c.code) { childLead = leadStore[j]; break; }
        }
        return {
          code:       c.code,
          status:     c.status,
          level:      c.inviteLevel || 1,
          clientName: childLead ? childLead.clientName : (c.friendName || null),
          ...
          children: all
            ...
            .map(function(cc) {
              var ccLead = null;
              for (var k = 0; k < leadStore.length; k++) {
                if (leadStore[k].code === cc.code) { ccLead = leadStore[k]; break; }
              }
              return {
                code:       cc.code,
                status:     cc.status,
                level:      cc.inviteLevel || 2,
                clientName: ccLead ? ccLead.clientName : (cc.friendName || null),
                booked:     !!cc.bookingSubmittedAt,
                sharedAt:   cc.sharedAt   || null,
                redeemedAt: cc.redeemedAt || null,
              };
            }),
        };
      });
    ...
    return {
      code:            entry.code,
      level:           entry.inviteLevel || 0,
      clientName:      lead ? lead.clientName : (entry.clientName || null),
      status:          entry.status,
      booked:          !!entry.bookingSubmittedAt,
      rewardPoints:    entry.rewardPoints    || 0,
      rewardsUnlocked: entry.rewardsUnlocked || [],
      children:        children,
    };
  }
  ...
  return res.json({
    tree:           tree,
    summary: {
      invitesSent:    l1.length,
      l1Booked:       l1.filter(function(c){ return c.booked; }).length,
      l2Booked:       l2.filter(function(c){ return c.booked; }).length,
      totalBooked:    l1.filter(function(c){ return c.booked; }).length + l2.filter(function(c){ return c.booked; }).length,
      conversionRate: l1.length > 0 ? Math.round((l1.filter(function(c){ return c.booked; }).length / l1.length) * 100) : 0,
      rewardPoints:   tree.rewardPoints,
    },
  });
});
...
app.get('/admin/invite-summary', requireAdmin, function(req, res) {
  ...
  all.filter(function(c) { return c.source === 'referral'; }).forEach(function(c) {
    ...
    if (!parent) return;
    if (!referrers[parent]) referrers[parent] = { sent: 0, l1Booked: 0, l2Booked: 0, rewardPoints: 0 };
    var r = referrers[parent];
    r.sent++;
    if (c.bookingSubmittedAt) {
      if ((c.inviteLevel || 1) === 1) r.l1Booked++;
      else r.l2Booked++;
    }
    var pEntry = codeStore.get(parent);
    if (pEntry) r.rewardPoints = pEntry.rewardPoints || 0;
  });

  var topReferrers = Object.keys(referrers).map(function(code) {
    var lead = null;
    for (var i = 0; i < leadStore.length; i++) {
      if (leadStore[i].code === code) { lead = leadStore[i]; break; }
    }
  ...
  return res.json({
    totalInvitesSent:  totalL1 + totalL2,
    totalL1:           totalL1,
    totalL2:           totalL2,
    l1BookingRate:     totalL1 > 0 ? Math.round(l1Booked / totalL1 * 100) : 0,
    l2BookingRate:     totalL2 > 0 ? Math.round(l2Booked / totalL2 * 100) : 0,
...
app.patch('/admin/reward/:code', requireAdmin, function(req, res) {
  ...
  var body = req.body || {};
  if (typeof body.rewardPoints === 'number') {
    entry.rewardPoints = body.rewardPoints;
  }
  if (body.addReward && typeof body.addReward === 'string') {
    if (!entry.rewardsUnlocked) entry.rewardsUnlocked = [];
    if (entry.rewardsUnlocked.indexOf(body.addReward) === -1) {
      entry.rewardsUnlocked.push(body.addReward);
    }
  }
  if (body.removeReward && typeof body.removeReward === 'string') {
    entry.rewardsUnlocked = (entry.rewardsUnlocked || []).filter(function(r) { return r !== body.removeReward; });
  }
  codeStore.set(code, entry);
  saveCodes(codeStore);
  pushActivity('Reward Updated', code + ' - ' + entry.rewardPoints + 'pts');
  return res.json({ ok: true, rewardPoints: entry.rewardPoints, rewardsUnlocked: entry.rewardsUnlocked });
});
